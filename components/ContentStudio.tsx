...
const ContentStudio: React.FC<Props> = ({ 
  ...
  // AUTO-TRIGGER MODAL: If we're in the studio without a business name, immediately ask for it.
  useEffect(() => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      handleOutputChange("Generation failed. Please ensure your API key is correct and try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getPlaceholder = () => {
    switch (activeTab) {
      ...
      case 'challenge': return "Describe the issue you are appealing (e.g., Wrongly suspended for 'Deceptive Content' after address change). Mention specific evidence like business license or utility bills...";
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center bg-white rounded-2xl border border-slate-200 shadow-sm p-12 text-center animate-fade-in">
        ...
        <p className="text-slate-600 max-w-md mb-8 leading-relaxed">
          The Content Studio uses your business name and industry to tailor AI responses. Please pick a business to continue.
        </p>
        <button 
        ...
        <div className={`${focusMode ? '' : 'lg:col-span-4'} flex flex-col space-y-4`}>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex-1 flex flex-col min-h-[350px]">
            ...
            <button 
              ...
              className="mt-4 w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 disabled:opacity-50 transition-all shadow-lg shadow-blue-600/20 active:scale-95"
            >
